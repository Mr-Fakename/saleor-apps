...
export default async function handler(
  ...
  const client = createClient(SALEOR_API_URL, async () => {
    ...
    if (!authData) {
      console.log("[add-to-cart] Auth data not found for configured URL, trying fallback to any auth data");
      const allAuth = await apl.getAll();
      if (allAuth.length > 0) {
        authData = allAuth[0];
        console.log("[add-to-cart] Using fallback auth data for:", authData.saleorApiUrl);
      }
    }
